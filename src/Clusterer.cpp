...
Clusterer::Clusterer(Configuration &config)
    : m_config(config)
{
                            ...
                            double chipTime, uint8_t geoId)
{
  ...
  if (pos0 == -1)
  {
    printf(DEB, "\t\tDetector or Plane not defined for FEC %d and vmmId %d!\n",
           (int)fecId, (int)vmmId);
    return true;
  }
  ...
  if (m_stats.GetDeltaTriggerTimestamp(fecId) > 0)
  {
    ...
    printf(DEB, "\tTime since last trigger %f us (%.4f kHz)\n",
  ...
  {
    printf(DEB, "\tDetector %d, plane %d, vmmId  %d\n", (int)det, (int)plane,
           vmmId);
  }
  printf(DEB, "\t\tId0 %d, id1 %d (chNo  %d) - overThresholdFlag %d\n", pos0,
         pos1, chNo, (int)overThresholdFlag);
  printf(DEB, "\t\t\tbcid %d, tdc %d, adc %d\n", bcid, tdc, adc & 0x3FF);
  printf(DEB, "\t\t\ttotal time %f, chip time %f ns\n", totalTime, chipTime);
  ...
  */
  while (cluster.size() > 0)
  {
    ...
    if (clusterFound.size() >= m_config.pMinClusterSize)
    {
      ...
      printf(DEB, "\tpos x/pos y: %f/%f", x_adc / adcTotal, y_adc / adcTotal);
      printf(DEB, "\ttime: %f", clusterPlane.time);
      printf(DEB, "\tadc: %u", clusterPlane.adc);
      printf(DEB, "\tsize: %u\n", clusterPlane.size);
      if (m_config.pShowStats)
      {
                              ...
                              ClusterContainer &cluster, double maxDeltaTime)
{
  ...
  for (auto &itCluster : cluster)
  {
    ...
    {
      printf(DEB, "\tstrip %d, time %llu, adc %d:\n", strip1,
             ...
             largestTime - time1 > m_config.pSpanClusterTime)
    {
      ...
      {
        printf(DEB, "******** INVALID CLUSTER SIZE ********%d\n\n", stripCount);
      }
      else
      {
        spanCluster = (largestTime - startTime);
    ...
    if (bestMatchPlane1 != end(m_clusters[dp1]))
    {
      ...
      if (found1 == true && found2 == true)
      {
        ...
        if (m_config.GetDetectorPlane(dp2) == true)
        {
          ...
          printf(DEB, "\tpos x/pos y/pos u: %f/%f/%f", clusterDetector.pos0,
                 clusterDetector.pos1, clusterDetector.pos2);
          printf(DEB, "\ttime x/time y/time u: %llu/%llu/%llu",
          ...
          printf(DEB, "\tadc x/adc y/adc u: %u/%u/%u", clusterDetector.adc0,
                 clusterDetector.adc1, clusterDetector.adc2);
          printf(DEB, "\tsize x/size y/size u: %u/%u/%u", clusterDetector.size0,
                 clusterDetector.size1, clusterDetector.size2);
          printf(DEB, "\tdelta time planes 0 - 1: %d\n",
                 (int)clusterDetector.delta_plane_0_1);
          printf(DEB, "\tdelta time planes 1 - 2: %d\n",
                 (int)clusterDetector.delta_plane_1_2);
          printf(DEB, "\tdelta time planes 0 - 2: %d\n",
        ...
        else
        {
          ...
          printf(DEB, "\tpos x/pos y: %f/%f", clusterDetector.pos0,
                 clusterDetector.pos1);
          printf(DEB, "\ttime x/time y: %llu/%llu",
                 static_cast<unsigned long long>(clusterDetector.time0),
                 static_cast<unsigned long long>(clusterDetector.time1));
          printf(DEB, "\tadc x/adc y: %u/%u", clusterDetector.adc0,
                 clusterDetector.adc1);
          printf(DEB, "\tsize x/size y: %u/%u", clusterDetector.size0,
                 clusterDetector.size1);
          printf(DEB, "\tdelta time planes: %d\n",
                 (int)clusterDetector.delta_plane_0_1);
        }
...
void Clusterer::AnalyzeClustersPlane(std::pair<uint8_t, uint8_t> dp)
{
  ...
  printf(DEB, "%d cluster in detector %d plane %d\n", cnt, (int)std::get<0>(dp),
         (int)std::get<1>(dp));
