...
/* Modified version of the original source code Configuration.cpp 
...
bool Configuration::ParseCommandLine(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i += 2) {
    std::cout<<"Simon argument Config: "<<argv[i]<<" "<<strncmp(argv[i], "-fecAndVmm", 10)<<std::endl;
    /* std::cout<<"Simon argument Config: "<<argv[i-1]<<std::endl; */
    /* std::cout<<"Simon argument Config: "<<strncmp(argv[i], "-geo", 4) <<std::endl; */
    
    if (strncmp(argv[i], "-fecAndVmm", 10) == 0) {
      std::string inputString = argv[i + 1];
      int fecId = -1;
      int vmmId = -1;
      std::string fecDelimiter = "][";
      std::string vmmDelimiter = ",";
      // Remove spaces
      ...
      std::stringstream fecStream(parameterString);
      std::string fecBuffer;
      while (std::getline(fecStream, fecBuffer, fecDelimiter[0])) {
          fecId++;
          //remove the remaining [ bracket in first position
          std::string cleanFecBuffer = fecBuffer.substr(1);
          std::stringstream vmmStream(cleanFecBuffer);
          std::string vmmBuffer;
          while (std::getline(vmmStream, vmmBuffer, vmmDelimiter[0])) {
            vmmId = stoi(vmmBuffer);
            pFecVmm[fecId][vmmId] = true;
            std::cout<<"Simon loop vmm "<<fecId<<" "<<vmmId<<" "<<pFecVmm[fecId][vmmId]<<std::endl;
          }
      }
    } 
    if (strncmp(argv[i], "-f", 2) == 0) {
      ...
      vmmsFound = true;
      /* std::cout<<"Simon vmmsfound "<<vmmsFound<<std::endl; */
      ...
      parameterString =
          std::regex_replace(parameterString, std::regex("\\],\\["), "|");
      std::cout<<"transfo : "<<parameterString<<std::endl;
    ...
    } else if (strncmp(argv[i], "-maxHits", 8) == 0) {
      std::string inputString = argv[i + 1];
      maxHits = stoi(inputString);
      std::cout<<"MAX HITS ---->> "<<inputString<<" "<<maxHits<<std::endl;
    } else {
...
bool Configuration::CalculateTransform() {
  /* std::cout<<"Simon - beg calculateTransform - "<<std::endl; */
  if (pTransform.size() == 0) {
    return true;
  }
  /* std::cout<<"Simon - mid calculateTransform - "<<std::endl; */
  for (auto &v : pTransform) {
    for (auto &e : v) {
  ...
  /* std::cout<<"Simon - end calculateTransform - "<<std::endl; */
  return true;
}
...
bool Configuration::CreateMapping() {
  if (pGeometryFile.find(".json") == std::string::npos && !vmmsFound) {
    return printf("Geometry definiton missing! Define geometry either via "
  ...
  /* std::cout<<"Simon - Configuration - mapping - 1 -: "<<vmmsFound<<" "<<std::endl; */
  if (vmmsFound) {
    int lastChip = 0;
    int chip = 0;
  ...
  /* std::cout<<"Simon - Configuration - mapping - 2 - det: "<<std::to_string(det)<<" plane: "<<std::to_string(plane)<<" lastChip: "<<std::to_string(lastChip)<<" chip: "<<std::to_string(chip)<<" "<<std::endl; */
      if (i % 2 == 1) {
        int flip = 0;
        if (lastChip > chip) {
    ...
    try {
      ...
      for (auto &geo : vmm_geos) {
        ...
        uint8_t plane = 0;
        /* std::cout<<"Simon - Configuration - mapping: fec "<<fec<<" vmm "<<vmm<<" detector "<<detector<<" strip0 "<<strips0<<" strip1 "<<strips1<<" "<<std::endl; */
...
//Read at txt file with the same arguments than for convertFile and reformat for ParseCommandLine
bool Configuration::SetConfiguration(std::string filename){
    std::cout<<"inside setConfiguration"<<std::endl;
    std::ifstream inputFile(filename);
    if (!inputFile.is_open()) {
        std::cerr << "Failed to open file with config parameters (same list as for convertFile)" << std::endl;
        return false;
    }
    ...
    while (std::getline(inputFile, line)) {
        ...
        while (startPos != std::string::npos && endPos != std::string::npos && startPos < endPos) {
            arguments.push_back(line.substr(startPos + 1, endPos - startPos - 1));
            std::cout << "Extracted text: " << line.substr(startPos + 1, endPos - startPos - 1) << std::endl;
    ...
    inputFile.close(); 
    // Convert vector of strings to char** format expected by ParseCommandLine
    std::vector<char*> argvVector;
    argvVector.reserve(arguments.size() + 1); // Reserve space for arguments + null pointer (last element of char** array) 
    for (const auto& arg : arguments) {
        argvVector.push_back(const_cast<char*>(arg.c_str())); // Add argument to the vector
    }
    argvVector.push_back(nullptr); // Add a nullptr at the end of the vector

    /* bool success = m_config.ParseCommandLine(argvVector.size() - 1, argvVector.data()); // Exclude nullptr from the count */
    bool success = this->ParseCommandLine(argvVector.size() - 1, argvVector.data()); // Exclude nullptr from the count
